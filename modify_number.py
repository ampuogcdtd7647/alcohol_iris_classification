...
from sklearn.metrics import f1_score, balanced_accuracy_score, cohen_kappa_score, classification_report, confusion_matrix
...
class AdaptiveFPRFNRControlledLoss(nn.Module):
    def __init__(self, base_loss, target_fpr=0.01, target_fnr=0.01, initial_alpha=1.0, initial_beta=1.0, learnable_alpha=True, learnable_beta=True):
        """
...
while not stop:
    ...
    if avg_val_loss < best_loss:
        ...
        best_epoch = epoch
        torch.save(model.state_dict(), '/data/hurong/iris_alcohol/best_model_2features.pth')
...
model.load_state_dict(torch.load('/data/hurong/iris_alcohol/best_model_2features.pth'))
model.eval()
correct = 0
total = 0
all_preds = []
...
# 计算 TNR (True Negative Rate) 和 TPR (True Positive Rate)
...
# 指定保存的文件夹路径和文件名
output_dir = r'/data/hurong/iris_alcohol/result'
